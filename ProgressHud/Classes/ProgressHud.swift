...
//  SVProgressHUD, https://github.com/ProgressHud/ProgressHud
//
...
public enum NotificationName : String {
    case ProgressHudDidReceiveTouchEvent, ProgressHudDidTouchDownInside, ProgressHudWillDisappear, ProgressHudDidDisappear, ProgressHudWillAppear, ProgressHudDidAppear, ProgressHudStatusUserInfoKey
...
public enum ProgressHudMaskType : Int {
    case none = 1
    case clear
    case black
    case gradient
...
public enum ProgressHudAnimationType : Int {
    case flat
    case native
}

private let ProgressHudParallaxDepthPoints : CGFloat = 10.0
private let ProgressHudUndefinedProgress : CGFloat = -1
private let ProgressHudDefaultAnimationDuration: CGFloat = 0.15
...
public class ProgressHud : UIView {
    ...
    private var defaultMaskType = ProgressHudMaskType.none
    private var defaultAnimationType = ProgressHudAnimationType.flat
    ...
    private var fadeInAnimationDuration: TimeInterval = TimeInterval(ProgressHudDefaultAnimationDuration)
    private var fadeOutAnimationDuration: TimeInterval = TimeInterval(ProgressHudDefaultAnimationDuration)
    ...
    private static let sharedView : ProgressHud = {
        ...
        if Thread.current.isMainThread {
            if ProgressHud.isNotAppExtension {
    ...
    @objc private func controlViewDidReceiveTouchEvent(_ sender: Any?, for event: UIEvent?) {
        NotificationCenter.default.post(name: NotificationName.ProgressHudDidReceiveTouchEvent.getNotificationName(), object: self, userInfo: notificationUserInfo())
        
        if let touchLocation = event?.allTouches?.first?.location(in: self) {
            if getHudView().frame.contains(touchLocation) {
                NotificationCenter.default.post(name:
                    NotificationName.ProgressHudDidTouchDownInside.getNotificationName(), object: self, userInfo: notificationUserInfo())
            }
        }
    ...
    func notificationUserInfo() -> [String : Any]? {
        if let statusText = getStatusLabel().text {
            return [NotificationName.ProgressHudStatusUserInfoKey.rawValue: statusText]
        }
        return nil
    }
    ...
    @objc private func fadeIn(_ object: AnyObject?) {
        ...
        if getBackGroundView().alpha != 1.0 {
            NotificationCenter.default.post(name: NotificationName.ProgressHudWillAppear.getNotificationName(), object: self, userInfo: notificationUserInfo())
            ...
            let completionBlock : () -> Void = {
                ...
                NotificationCenter.default.post(name: NotificationName.ProgressHudDidAppear.getNotificationName(), object: self, userInfo: self.notificationUserInfo())
    ...
    @objc private func positionHUD(_ notification: Notification? = nil) {
        ...
        if ProgressHud.isNotAppExtension == false {
        ...
        if getControlView().superview == nil {
            if containerView != nil {
                ...
                if ProgressHud.isNotAppExtension {
                    ...
                    // If ProgressHud is used inside an app extension add it to the given view
                    if viewForExtension != nil {
    ...
    private func show(image: UIImage, status: String?, duration: TimeInterval) {
        OperationQueue.main.addOperation({ [weak self] in
            ...
            strongSelf.progress = Float(ProgressHudUndefinedProgress)
            strongSelf.cancelRingLayerAnimation()
            strongSelf.cancelIndefiniteAnimatedViewAnimation()
    ...
    private func dismissWithDelay(_ delay: TimeInterval, completion: (() -> Void)?) {
        OperationQueue.main.addOperation({ [weak self] in
            ...
            NotificationCenter.default.post(name: NotificationName.ProgressHudWillDisappear.getNotificationName(), object: nil, userInfo: strongSelf.notificationUserInfo())
            ...
            let completionBlock: (() -> Void) = {
                ...
                if strongSelf.getBackGroundView().alpha == 0.0 {
                    ...
                    // Post notification to inform user
                    //ProgressHudDidDisappearNotification
                    NotificationCenter.default.post(name: NotificationName.ProgressHudDidDisappear.getNotificationName(), object: strongSelf, userInfo: strongSelf.notificationUserInfo())
    ...
    private func updateHUDFrame() {
        ...
        if CGFloat(progress) != ProgressHudUndefinedProgress {
...
extension ProgressHud {
    ...
    public class func set(defaultMaskType maskType: ProgressHudMaskType) {
        sharedView.defaultMaskType = maskType
    }
    
    public class func set(defaultAnimationType type: ProgressHudAnimationType) {
    ...
    public class func set(viewForExtension view: UIView) {
        ProgressHud.isNotAppExtension = false
    ...
    public class func show(withStatus status: String?) {
        show(progress: ProgressHudUndefinedProgress, status: status)
    }
